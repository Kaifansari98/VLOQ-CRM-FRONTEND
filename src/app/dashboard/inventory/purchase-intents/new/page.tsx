...
> = {
  Low: {
    label: "Low",
    icon: "○",
    active: "bg-slate-900 text-white border-slate-900 dark:bg-white dark:text-slate-900",
    ...
    >
      ...
      >
              ...
              {products.map((prod) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isSelected ? <CheckCircle2 size={17} /> : <Package size={16} />}
                    </div>

                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold">{prod.product_name}</p>
        ...
        <div className="flex items-center justify-between border-t bg-muted/20 px-5 py-3">
          ...
          <Button onClick={onClose} className="rounded-xl bg-indigo-600 hover:bg-indigo-700">
            Done
          </Button>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Select supplier for this product.</p>
          </div>
          <button
            onClick={onClose}
            className="rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground"
        ...
        <div className="overflow-y-auto p-2">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">{q ? "No supplier found" : "All suppliers already added"}</p>
            </div>
          ) : (
            filtered.map((v) => (
              <button
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold">{v.company_name}</p>
                  <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{v.vendor_code}</span>

                    {v.defaultPaymentTerm?.term_name && (
...
function ProductIntentCard({
  ...
  const productTotal = item.vendor_entries.reduce((s, e) => s + toNum(e.total_amount), 0);
  const totalQty = item.vendor_entries.reduce((s, e) => s + toNum(e.required_qty), 0);
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-base font-semibold">{item.product.product_name}</p>
                  <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    {item.product.unit_of_measure && <span>{item.product.unit_of_measure}</span>}
                  </div>
                </div>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-3">
                <div className="rounded-2xl bg-muted/40 p-3">
                  ...
                  <p className="mt-1 text-sm font-bold">{item.vendor_entries.length}</p>
                </div>
                <div className="rounded-2xl bg-muted/40 p-3">
                  <p className="text-[10px] font-black uppercase text-muted-foreground">
                    Total Qty
          ...
          <div className="border-t bg-muted/20 p-4 md:p-5">
            <div className="grid gap-3 md:grid-cols-[180px_1fr]">
              <div>
                ...
                />
                {item.errors?.uom && (
                  <p className="mt-1 text-xs font-medium text-red-500">{item.errors.uom}</p>
                )}
              </div>
            ...
            {item.product.hsn_code && (
              <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50/60 px-3 py-2 text-xs text-blue-700 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-300">
                <ShieldCheck size={14} />
                <span className="font-semibold">HSN detected</span>
                <span>HSN {item.product.hsn_code}</span>
            ...
            <div className="mt-5">
                ...
                <div>
                  <p
                    className={cn(
                      ...
                      item.errors?.no_vendors ? "text-red-500" : "text-muted-foreground"
                    )}
                  >
                    Suppliers *
                  </p>
                  {item.errors?.no_vendors && (
                    <p className="text-xs font-medium text-red-500">{item.errors.no_vendors}</p>
                  )}
                </div>
              ...
              {item.vendor_entries.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-xs">Quantity, rate, GST and delivery date will be added here.</p>
                </button>
              ) : (
                <div className="overflow-hidden rounded-2xl border bg-background">
                  {/* <div className="grid grid-cols-[1fr_80px_104px_1fr_24px] gap-1.5 border-b bg-muted/40 px-3 py-2 text-[9px] font-black uppercase text-muted-foreground">
                    <span>Supplier</span>
                    <span>Qty *</span>
                    <span>Req. By *</span>
                    <span>Remarks</span>
                  ...
                  <div className="grid grid-cols-[80px_70px_82px_70px_70px_70px_90px_100px] gap-1.5 border-b bg-muted/20 px-3 py-2 text-[9px] font-black uppercase text-muted-foreground">
                    <span>MRP</span>
                    <span>Disc%</span>
                    <span>Rate *</span>
                    <span>GST%</span>
...
export default function RaisePurchaseIntentPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([categories, stateId, vendors, terms]) => {
  ...
  : "";
        ...
        // compare state ids
        const isSameState = Number(vendor.state_id) === Number(state_id);

        if (isSameState) {
          // Intra-state → CGST + SGST
          entry.cgst_pct = String(item.product.cgst_rate || 0);
          entry.sgst_pct = String(item.product.sgst_rate || 0);
          entry.igst_pct = "0";
          entry.tax_pct = String(
            Number(item.product.cgst_rate || 0) +
            Number(item.product.sgst_rate || 0)
          );
        } else {
          // Inter-state → IGST only
          entry.cgst_pct = "0";
          entry.sgst_pct = "0";
          entry.igst_pct = String(item.product.tax_pct || 0);
          entry.tax_pct = String(item.product.tax_pct || 0);
        }

        const calculatedEntry = recalcVendorEntry(entry);

        item.vendor_entries = [...item.vendor_entries, calculatedEntry];
  ...
  const removeVendorFromItem = useCallback((itemIdx: number, vi: number) => {
    setSelectedItems((prev) => {
      ...
      next[itemIdx] = {
        ...next[itemIdx],
        vendor_entries: next[itemIdx].vendor_entries.filter((_, index) => index !== vi),
      };
  ...
  const validate = (): boolean => {
    ...
    const itemsWithErrors = selectedItems.map((item) => {
      ...
      const vendorErrors: VendorEntryErrors[] = item.vendor_entries.map((entry) => {
        ...
        // if (!entry.required_by_date) {
        //   ve.required_by_date = "Required";
        //   valid = false;
        // }
  ...
  const handleSubmit = async () => {
    ...
    if (!validate()) {
      toastManager.add({ title: "Fix errors before submitting", type: "error" });
      return;
    }
...
payment_term_id: entry.payment_term_id
  ...
  total_amount: toNum(entry.total_amount) || null,
      ...
      toastManager.add({ title: "Purchase Intent raised!", type: "success" });
      router.push("/dashboard/inventory/purchase-intents");
    } catch {
      toastManager.add({ title: "Failed to raise PI", type: "error" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const supplierCount = selectedItems.reduce((s, item) => s + item.vendor_entries.length, 0);
    const grandTotal = selectedItems.reduce(
      (s, item) => s + item.vendor_entries.reduce((ss, entry) => ss + toNum(entry.total_amount), 0),
      0
    );
            ...
            <div className="relative p-5 md:p-6">
              ...
              <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                    Select products, compare suppliers, calculate landed cost and submit the
                    purchase intent in one clean workflow.
                  </p>
                </div>
                  ...
                  {PRIORITIES.map((p) => (
                    <button
                      ...
                      className={cn(
                        ...
                        priority === p ? PRIORITY_META[p].active : PRIORITY_META[p].idle
                      )}
                    >
              ...
              <div className="relative mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-2xl border bg-muted/30 p-4">
                  ...
                  <p className="mt-1 text-xl font-black">{stats.supplierCount}</p>
                </div>
                ...
                <div className="rounded-2xl border bg-muted/30 p-4">
                  ...
                  >
                    {stats.errorCount > 0
                      ? `${stats.errorCount} item${stats.errorCount > 1 ? "s" : ""} need fixing`
            ...
            <section className="space-y-4">
              <div className="flex flex-col gap-3 rounded-3xl border bg-background p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Add products and configure supplier-wise requirement.
                  </p>
                </div>
              ...
              {selectedItems.length === 0 ? (
                <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[28px] border border-dashed bg-background p-8 text-center shadow-sm">
                  ...
                  <h2 className="text-lg font-semibold">Build your purchase intent</h2>
                  <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                    Start by selecting a category and adding products. Supplier pricing and taxes
                    can be configured after selection.
                  </p>
                  <Button
                    onClick={() => setProductPickerOpen(true)}
            ...
            <aside className="lg:sticky lg:top-6 lg:h-fit">
              <div className="overflow-hidden rounded-[28px] border bg-background shadow-sm">
                <div className="border-b p-5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Review before submitting purchase intent.
                  </p>
                </div>
                ...
                <div className="border-t p-5">
                  {stats.errorCount > 0 && (
                    <div className="mb-3 rounded-2xl border border-red-200 bg-red-50 p-3 text-xs font-medium text-red-600 dark:border-red-900 dark:bg-red-950/30">
                      Please fix highlighted items before submitting.
                    </div>
                  )}
                  ...
                  >
                    ...
                    {submitting ? "Raising Intent..." : "Raise Purchase Intent"}
                  </Button>
                </div>
              </div>
            </aside>
