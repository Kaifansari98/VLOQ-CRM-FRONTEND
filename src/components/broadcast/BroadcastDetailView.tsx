...
export const BroadcastDetailView: React.FC<BroadcastDetailViewProps> = ({
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-5.5rem)] space-y-4 font-sans">
      ...
      {/* 2. Main 70/30 Responsive Split Layout */}
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-4 w-full flex-1 items-start">
        {/* Left Side (70%) - Broadcast Content */}
        <div className="lg:col-span-7 border rounded-2xl bg-card p-5 shadow-2xs flex flex-col h-full min-h-[480px]">
          <div className="flex items-center justify-between pb-3.5 mb-3.5 border-b shrink-0">
            ...
            {item.category && (
              <span className="text-[11px] font-semibold text-muted-foreground px-2.5 py-1 rounded-md bg-muted border">
                Category: <strong className="text-foreground">{item.category}</strong>
              </span>
            )}
          </div>

          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-22rem)] pr-2 text-sm leading-relaxed text-foreground space-y-4 rich-text-container bg-muted/10 p-5 rounded-xl border border-border/30">
        ...
        {/* Right Side (30%) - Document Attachments Panel Only */}
        <div className="lg:col-span-3 border rounded-2xl bg-card p-5 shadow-2xs flex flex-col h-full min-h-[480px]">
          <div className="flex items-center justify-between pb-3.5 mb-3.5 border-b shrink-0 gap-2 flex-wrap">
            ...
            {isSuperAdmin && (
              <Button
                ...
                className="h-7 text-[10px] font-semibold text-primary bg-primary/10 hover:bg-primary/20 border-primary/20 gap-1 rounded-lg cursor-pointer px-2"
              >
          ...
          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-22rem)] space-y-4 pr-1">
            {(item.attachments?.length ?? 0) === 0 ? (
              <div className="p-6 border rounded-xl border-dashed text-center text-xs text-muted-foreground bg-muted/10 flex flex-col items-center justify-center gap-2.5 h-full min-h-[220px]">
                <FileArchive className="w-8 h-8 text-muted-foreground/40" />
                <span className="font-medium text-center">No document attachments attached to this broadcast</span>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5 text-primary" /> Files ({item.attachments?.length})
                  </h4>
                ...
                <div className={attachmentViewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 gap-2.5" : "flex flex-col space-y-2"}>
      ...
      {/* 3. Bottom Section - Video / YouTube Media (Full Width at Bottom) */}
      {item.videoLinks && item.videoLinks.length > 0 && (
        <div className="border rounded-2xl bg-card p-5 shadow-2xs w-full space-y-4">
          <div className="flex items-center justify-between pb-3 border-b shrink-0">
            <h3 className="text-xs font-bold uppercase tracking-wider text-foreground flex items-center gap-2 m-0">
              <span className="p-1.5 rounded-lg bg-primary/10 text-primary">
                <Video className="w-4 h-4" />
              </span>
              Video Media & Tutorials ({item.videoLinks.length})
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {item.videoLinks.map((url, idx) => {
              ...
              return (
                <div key={idx} className="border rounded-2xl overflow-hidden bg-card p-3 space-y-2 shadow-2xs">
                  {embedUrl ? (
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-2 text-xs font-semibold text-primary underline p-2"
                    >
