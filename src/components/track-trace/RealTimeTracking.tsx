      ...
      >
        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item.id}
              className="flex items-start gap-3 px-3.5 py-3 rounded-xl border border-border bg-background hover:bg-muted/30 transition-colors duration-150"
            >
              {/* Index badge */}
              ...
              <div className="flex-1 min-w-0">
                {/* Row 1: Project · lead code  ───────  status pill + scanned time */}
                <div className="flex items-center justify-between gap-2 mb-0.5">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded bg-muted text-muted-foreground border border-border leading-none shrink-0">
                      {item.lead.lead_code}
                    </span>
                  </div>

                  {/* Status pill + scanned time together */}
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full border text-[9px] font-semibold ${cfg.pill}`}
                    >
                      <span
                        className={`w-1 h-1 rounded-full shrink-0 ${cfg.dot} ${item.status === "in_process" ? "animate-pulse" : ""}`}
                      />
                      {cfg.label}
                    </span>
                    {item.actual_in_at_formatted && (
                      <>
                        ...
                        <span className="text-[9px] tabular-nums text-muted-foreground font-medium leading-none">
                          {item.actual_in_at_formatted}
                        </span>
                      </>
                    )}
                ...
                {/* Row 2: Project name */}
                <p className="text-xs font-semibold text-foreground leading-snug mb-2 truncate">
                  {item.project.project_name}
                </p>

                {/* Row 3: Item · Material · Machine · Operator — 4 fixed cols, no wrap */}
                <div className="grid grid-cols-[4fr_4fr_1fr_1fr] gap-x-3 mb-1.5">
                  <div className="min-w-0">
                    <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-widest leading-none mb-0.5">
                      Item
                    </p>
                    <p className="text-[11px] text-foreground font-medium leading-none truncate">
                ...
                <div className="flex items-center gap-0.5 min-w-0">
                  <span className="text-[9px] font-semibold text-muted-foreground uppercase tracking-widest leading-none shrink-0">
                    Description : 
                  </span>
                  <span className="text-border text-[9px] shrink-0">·</span>
                  <p className="text-[11px] text-foreground font-medium leading-none truncate">
