...
import {
  orderLoginLead as DispatchStageLead,
  ProcessedOrderLoginLead as ProcessedDispatchStageLead,
...
const DispatchStageTable = () => {
  ...
  const [rowAction, setRowAction] =
    useState<DataTableRowActionFinalMeasurement<ProcessedDispatchStageLead> | null>(
      null
    );
  ...
  // 🔹 Data Mapping
  const rowData = useMemo<ProcessedDispatchStageLead[]>(() => {
    if (!data?.leads || !Array.isArray(data.leads)) return [];

    return (data.leads as DispatchStageLead[]).map((lead, index) => ({
      id: lead.id,
      srNo: index + 1,
  ...
  const handleRowClick = (row: ProcessedDispatchStageLead) => {
