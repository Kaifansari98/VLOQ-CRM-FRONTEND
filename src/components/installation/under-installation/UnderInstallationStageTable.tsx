...
const UnderInstallationStageTable = () => {
  ...
  // 🔹 Fetch Under Installation Stage Data
  const { data, isLoading, isError } = useUnderInstallationStageLeads(
    vendorId!,
    userId!,
  ...
  const [rowAction, setRowAction] =
    useState<DataTableRowActionFinalMeasurement<ProcessedUnderStageLead> | null>(
      null
    );
  ...
  // 🔹 Data Mapping
  const rowData = useMemo<ProcessedUnderStageLead[]>(() => {
    if (!data?.leads || !Array.isArray(data.leads)) return [];

    return (data.leads as UnderStageLead[]).map((lead, index) => ({
      id: lead.id,
      srNo: index + 1,
  ...
  const handleRowClick = (row: ProcessedUnderStageLead) => {
    router.push(`/dashboard/installation/under-installation/details/${row.id}`);
  };

  if (!vendorId) return <p>No vendor selected</p>;
  if (isLoading) return <p>Loading Under Installation leads...</p>;
  if (isError) return <p>Error loading Under Installation leads</p>;
...
export default UnderInstallationStageTable;
