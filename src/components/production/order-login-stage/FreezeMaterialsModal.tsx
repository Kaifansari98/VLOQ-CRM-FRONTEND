...
function rowLimits(row: ProductionPreviewRow) {
  ...
  // Can't freeze past whichever is further along — direct issuing (without freezing first)
  // also counts against how much of the requirement is left to freeze.
  const remaining = Math.max(0, round2(required - Math.max(frozen, issued)));
  ...
  return { required, frozen, issued, remaining, available, max };
}
            ...
            <div key={row.key} className={cn("rounded-xl border p-3.5 transition-opacity", limits.max <= 0 && "opacity-60")}>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="shrink-0 text-right text-xs text-muted-foreground">
                  ...
                  {!!limits.frozen && <p>Already frozen: <span className="font-medium text-foreground">{limits.frozen} {row.unit}</span></p>}
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                ...
                {limits.max <= 0 ? (
                  <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-muted-foreground dark:border-slate-700 dark:bg-slate-900">
                    {limits.remaining <= 0 ? "Nothing left to freeze" : "No stock available"}
                  </span>
                ) : (
